...
export const updateCollections = async (req, res) => {
    try {
        const { collectionType, movieTitle } = req.body;
        const userId = req.user._id;

        // Find the user and await the result
        const currentUser = await User.findById(userId);
        if (!currentUser) {
            return res.status(404).json({ message: "User not found" });
        }

        const isInCollection = currentUser.collections.get(collectionType).includes(movieTitle);

        if (!isInCollection) {
            currentUser.collections.get(collectionType).push(movieTitle);
        } else {
...
export const deleteCollections = async (req, res) => {
    try {
        ...
        if (isInCollection) {
            currentUser.collections.get(collectionType).splice(currentUser.collections.get(collectionType).indexOf(movieTitle), 1);
        } else {
            res.status(400).json({ message: "Movie not in collection" });
            return;
        }
